...
(function(){
  ...
  /* ==================== 背景音乐 ==================== */
  // 移动端浏览器禁止无用户手势的音频播放，以下用多种策略最大化自动播放成功率
  var bgm1 = $('bgm1');
  var bgm2 = $('bgm2');
  ...
  function startBGM1() {
    if (_bgmStarted || !bgm1) return;
    _bgmStarted = true;
    // 不用 currentTime=0，保持 autoplay 可能已有的进度
    var p = bgm1.play();
  ...
  function cleanup() {
    document.removeEventListener('click', startBGM1);
    document.removeEventListener('touchstart', startBGM1);
    document.removeEventListener('WeixinJSBridgeReady', startBGM1);
  }
  ...
  document.addEventListener('WeixinJSBridgeReady', startBGM1);

  // 策略2：DOM 解析完立即尝试（桌面端 Chrome 通常允许）
  bgm1.play().then(function() {
    _bgmStarted = true;
    cleanup();
  }).catch(function() {
    // 被拦截，等待用户手势
  ...
  // 策略3：用户首次触摸/点击即启动
  document.addEventListener('click', startBGM1);
  document.addEventListener('touchstart', startBGM1);
